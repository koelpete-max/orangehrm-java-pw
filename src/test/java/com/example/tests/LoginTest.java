...
public class LoginTest extends BaseTest {
    ...
    public void validCredentialsShouldGrantUserAccessToTheSystemTest(TestUser user) {
        ...
        Assert.assertTrue(
                !loginPage.isInvalidCredentialsMessageVisible(timeout) &&
                        topbarPanel.getPanelText().equals(TopbarPanelText.DASHBOARD.toString()),
                "No login possible for the user " + user.username()
        );
        Assert.assertEquals(
    ...
    public void invalidCredentialsShouldNotAllowUserToAccessTheSystemTest()  {
        ...
        var testUser = TestUserProvider.get("InvalidUser");
